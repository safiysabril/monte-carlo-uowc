...
def _(
    SimConfig, SIM, os,
    n_photons_ui, min_captured_ui, link_ranges_ui,
):
    cfg = SimConfig(
...
@app.cell
def _(mo):
    run_btn = mo.ui.button(label="▶  Run Simulation", kind="success")
    return (run_btn,)
...
# Note: mo.stop halts this cell (and all downstream cells) until the button
# has been clicked at least once, preventing accidental long runs on load.
# ─────────────────────────────────────────────────────────────────────────────
...
def _(
    ...
    return raw_hom, raw_inh, sim_beams, sim_media, sim_waters
    ...
    for _water in sim_waters:
        for _beam in sim_beams:
            for _Z in cfg.link_ranges_m:
                ...
                metrics_hom[_key] = compute_all_metrics(
                    raw_hom[_key], cfg, _water.c, _Z,
                )
    ...
    for _medium in sim_media:
        for _beam in sim_beams:
            for _Z in cfg.link_ranges_m:
                ...
                metrics_inh[_key] = compute_all_metrics(
                    raw_inh[_key], cfg, _medium.c_max, _Z,
                )
...
@app.cell
def _(mo):
    mo.md("## 📊  Channel Figures")
    return


# ── Shared plot styling helpers (local to this cell, returned for reuse) ─────

@app.cell
def _(plt, ticker):
    _GRID_KW  = dict(alpha=0.25, linestyle="--")
    _LINE_KW  = dict(linewidth=1.8, markersize=6)
    ...
    def _semilogy_finite(ax, x, y, **kw):
        """Plot only finite (non-NaN, non-inf) points on a semilogy axis."""
        import numpy as _np
        xf = [xi for xi, yi in zip(x, y) if _np.isfinite(yi) and yi > 0]
        yf = [yi for yi in y if _np.isfinite(yi) and yi > 0]
        if xf:
            ax.semilogy(xf, yf, **kw)

    return _COLOURS, _GRID_KW, _LINE_KW, _MARKERS, _semilogy_finite, _style_ax
...
@app.cell
def _(
    mo, plt, cfg, RunKey, metrics_hom, metrics_inh,
    sim_waters, sim_beams, sim_media,
    _COLOURS, _MARKERS, _LINE_KW, _style_ax,
):
    mo.md("### Fig 1 — Received Power vs Link Range")
    return
...
def _(
    ...
    _fig1, _axes = plt.subplots(1, _n_panels, figsize=(6.5 * _n_panels, 5),
                                 squeeze=False)
    _fig1.suptitle("Normalised Received Power vs Link Range", fontsize=12, fontweight="bold")

    # Homogeneous panels — one per water type
    for _i, _water in enumerate(sim_waters):
        ...
        for _j, _beam in enumerate(sim_beams):
            ...
            _bl  = [metrics_hom[RunKey(_water.name, _beam.name, float(Z))]["beer_lambert_dB"]
                    for Z in _ranges]
            _ax.plot(_ranges, _pwr, marker=_MARKERS[_j], color=_COLOURS[_j],
                     label=f"{_beam.name} (MC)", **_LINE_KW)
            _ax.plot(_ranges, _bl, "--", color=_COLOURS[_j], alpha=0.5,
                     label=f"{_beam.name} (Beer-Lambert)")
        _style_ax(_ax, "Link Range (m)", "Normalised Power (dB)", _water.name)
        ...
        for _j, _medium in enumerate(sim_media):
            for _k, _beam in enumerate(sim_beams):
                ...
                _ax.plot(_ranges, _pwr,
                         marker=_MARKERS[_k], color=_COLOURS[_j],
                         linestyle="-" if _k == 0 else "--",
                         label=f"{_medium.name[:20]} | {_beam.name[:12]}",
                         **_LINE_KW)
        _style_ax(_ax, "Link Range (m)", "Normalised Power (dB)", "Inhomogeneous Media")
...
@app.cell
def _(mo):
    mo.md("### Fig 2 — Channel Impulse Response (CIR)")
    return
    ...
    for _r, (_name, _kind) in enumerate(_entities):
        for _b, _beam in enumerate(sim_beams):
            for _col_off, _Z in enumerate([_z_near, _z_far]):
                ...
                _t  = _m["t_axis"] * 1e9    # → ns
                _h  = _m["cir"]
                _ax.fill_between(_t, _h, alpha=0.35, color=_COLOURS[_b])
                _ax.plot(_t, _h, color=_COLOURS[_b], linewidth=1.2)
                _style_ax(
...
@app.cell
def _(mo):
    mo.md("### Fig 3 — Channel Frequency Response |H(f)|")
    return
    ...
    for _r, (_name, _kind) in enumerate(_entities):
        for _b, _beam in enumerate(sim_beams):
            for _col_off, _Z in enumerate([_z_near, _z_far]):
                ...
                _ax.plot(_f, _H, color=_COLOURS[_b], linewidth=1.4)
...
@app.cell
def _(mo):
    mo.md("### Fig 4 — RMS Delay Spread vs Link Range")
    return
...
def _(
    ...
    for _b, _beam in enumerate(sim_beams):
        ...
        for _name, _kind, _idx in _all_entities:
            ...
            _semilogy_finite(_ax, _ranges, _ds,
                             marker=_MARKERS[_idx % len(_MARKERS)],
                             color=_COLOURS[_idx % len(_COLOURS)],
                             label=_lbl, **_LINE_KW)
        _style_ax(_ax, "Link Range (m)", "RMS Delay Spread (s)", _beam.name)
        _ax.legend(fontsize=8)
        _ax.yaxis.set_major_formatter(
            ticker.FuncFormatter(lambda y, _: f"{y:.1e}")
        )
...
@app.cell
def _(mo):
    mo.md("### Fig 5 — 3 dB Channel Bandwidth vs Link Range")
    return
    ...
    for _b, _beam in enumerate(sim_beams):
        ...
        for _name, _kind, _idx in _all_entities:
            ...
            _bw  = [_src[RunKey(_name, _beam.name, float(Z))]["bandwidth_hz"] / 1e6
                    for Z in _ranges]
            _lbl = f"{_name[:28]} ({'hom' if _kind == 'hom' else 'inh'})"
            _semilogy_finite(_ax, _ranges, _bw,
                             marker=_MARKERS[_idx % len(_MARKERS)],
                             color=_COLOURS[_idx % len(_COLOURS)],
                             label=_lbl, **_LINE_KW)
        _style_ax(_ax, "Link Range (m)", "Bandwidth (MHz)", _beam.name)
        _ax.legend(fontsize=8)
        _ax.yaxis.set_major_formatter(
...
def _(
    ...
    for _medium_name, _beam_name, _kind in _all_combos:
        _src = metrics_hom if _kind == "Homogeneous" else metrics_inh
        for _Z in cfg.link_ranges_m:
            _key = RunKey(_medium_name, _beam_name, float(_Z))
            if _key not in _src:
                continue
            _m = _src[_key]
            _rows.append({
    ...
    mo.ui.table(_df_metrics, pagination=True, page_size=15)
...
def _(to_dataframe, raw_hom, raw_inh):
    # Merge homogeneous and inhomogeneous results into one DataFrame
...
def _(mo, filter_medium, filter_beam, filter_range):
    mo.hstack([filter_medium, filter_beam, filter_range], gap=3)
    return


@app.cell
def _(photon_df, filter_medium, filter_beam, filter_range, np, mo):
    _mask = (
...
@app.cell
def _(mo):
    mo.md("### Capture Statistics")
    return


@app.cell
def _(mo, filtered_df, raw_hom, raw_inh, RunKey, capture_statistics_with_launched):
    _all_raw = {**raw_hom, **raw_inh}
    ...
    mo.ui.table(_stats.round(4), pagination=False)
...
@app.cell
def _(mo):
    mo.md("### Photon Weight Distribution")
    return


@app.cell
def _(plt, np, filtered_df, _COLOURS, _style_ax):
    ...
    for _i, _Z in enumerate(_ranges):
        ...
        _cx = 0.5 * (_e[:-1] + _e[1:])
        _ax.semilogx(_cx, _h / _h.max(), color=_cmap(_i / max(len(_ranges) - 1, 1)),
                     linewidth=1.4, label=f"{_Z:.0f} m")

    _style_ax(_ax, "Photon Weight (log scale)", "Normalised Count",
...
@app.cell
def _(mo):
    mo.md("### Receiver Plane Spatial Distribution")
    return


@app.cell
def _(plt, np, filtered_df, _COLOURS):
    _ranges = sorted(filtered_df["link_range_m"].unique())
    _n = len(_ranges)
    _fig_sp, _axes = plt.subplots(1, max(_n, 1), figsize=(4 * max(_n, 1), 4), squeeze=False)
    ...
    for _i, _Z in enumerate(_ranges):
        ...
        _ax.imshow(
            _h.T, origin="lower", aspect="equal",
...
@app.cell
def _(mo):
    mo.md("### Time-of-Flight Distributions")
    return


@app.cell
def _(plt, np, filtered_df, tof_histograms, _style_ax):
    _ranges = sorted(filtered_df["link_range_m"].unique())
    ...
    _style_ax(_ax, "Time of Flight (ns)", "Normalised Weight Density",
...
@app.cell
def _(mo):
    mo.md("### Scattering Event Count vs Link Range")
    return


@app.cell
def _(plt, filtered_df, _COLOURS, _style_ax):
    _ranges = sorted(filtered_df["link_range_m"].unique())
    _cmap   = plt.colormaps["plasma"].resampled(max(len(_ranges), 1))
    ...
    for _i, _Z in enumerate(_ranges):
        ...
        _ax.hist(_ns, bins=_bins,
                 density=True, alpha=0.55,
                 color=_cmap(_i / max(len(_ranges) - 1, 1)),
                 label=f"{_Z:.0f} m", histtype="stepfilled")

    _style_ax(_ax, "Number of Scattering Events", "Density",
...
@app.cell
def _(mo):
    mo.md("### Excess Path Length vs Scatter Count")
    return


@app.cell
def _(plt, np, filtered_df, _COLOURS, _MARKERS, _style_ax):
    _ranges = sorted(filtered_df["link_range_m"].unique())
    _fig_ep, _ax = plt.subplots(figsize=(9, 4))

    for _i, _Z in enumerate(_ranges):
        _sub = filtered_df[filtered_df["link_range_m"] == _Z].sample(
            min(2_000, len(filtered_df[filtered_df["link_range_m"] == _Z])),
            random_state=42,
        )
        if len(_sub) < 5:
            continue
    ...
    _style_ax(_ax, "Number of Scattering Events", "Excess Path Length (m)",
